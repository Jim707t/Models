...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for val_x in self.domains[x]:
            is_consistent = any(val_x[i] == val_y[j] for val_y in self.domains[y])
            if not is_consistent:
                values_to_remove.append(val_x)
    ...
    def order_domain_values(self, var, assignment):
        ...
        for value in values:
            ...
            counts.append((value, ruled_out_count))

        counts.sort(key=lambda x: x[1])
        ordered_values = [value for value, _ in counts]
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in unassigned:
            if len(self.domains[var]) < len(self.domains[result_var]):
                result_var = var
            elif len(self.domains[var]) == len(self.domains[result_var]):
